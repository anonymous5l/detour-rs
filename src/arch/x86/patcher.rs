...
impl Patcher {
    ...
    pub fn new(target: *const (), detour: *const (), prolog_size: usize) -> Result<Patcher> {
    ...
    fn patch_area(target: *const (), prolog_size: usize) -> Result<&'static mut [u8]> {
        ...
        if !Self::is_patchable(target, prolog_size, jump_rel32_size) {
            ...
            if Self::is_patchable(target, prolog_size, jump_rel08_size) {
                ...
                let hot_patch_area =
                    unsafe { slice::from_raw_parts(hot_patch as *const u8, jump_rel32_size) };
                ...
                Ok(unsafe { slice::from_raw_parts_mut(hot_patch as *mut u8, patch_size) })
            } else {
                Err(Error::NoPatchArea)
            }
        } else {
            // The range is from the start of the function to the end of the jump
            Ok(unsafe { slice::from_raw_parts_mut(target as *mut u8, jump_rel32_size) })
        }
    }
    ...
    fn is_patchable(target: *const (), prolog_size: usize, patch_size: usize) -> bool {
