...
impl Disassembler {
    ...
    extern "C" fn udis_read_address(ud: *mut udis::ud) -> libc::c_int {
        unsafe {
...
impl Instruction {
    ...
    pub fn new(disasm: &mut Disassembler, address: *const ()) -> Option<Self> {
        let instruction_bytes = unsafe { udis::ud_disassemble(&mut disasm.0) as usize };
        if instruction_bytes > 0 {
            Some(Instruction {
                address: address as usize,
                mnemonic: unsafe { udis::ud_insn_mnemonic(&disasm.0) },
                operands: disasm.0.operand.to_vec(),
                bytes: unsafe { slice::from_raw_parts(address as *const _, instruction_bytes) },
            })
        } else {
            None
        }
