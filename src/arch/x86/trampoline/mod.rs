...
impl Trampoline {
    ...
    pub fn new(target: *const (), margin: usize) -> Result<Trampoline> {
...
impl Builder {
    ...
    /// ```
    fn handle_rip_relative_instruction(
        &mut self,
        instruction: &Instruction,
        displacement: isize,
    ) -> Result<Box<dyn pic::Thunkable>> {
        ...
        Ok(Box::new(pic::UnsafeThunk::new(
            move |offset| {
                ...
                let as_bytes: [u8; 4] = unsafe { mem::transmute(adjusted_displacement as u32) };
