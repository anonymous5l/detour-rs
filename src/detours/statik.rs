...
pub struct StaticDetour<T: Function> {
    closure: AtomicPtr<Box<dyn Fn(T::Arguments) -> T::Output>>,
...
impl<T: Function> StaticDetour<T> {
    ...
    /// # }
    /// ```
    pub fn initialize<D>(&self, target: T, closure: D) -> Result<&Self>
    where
        D: Fn(T::Arguments) -> T::Output + Send + 'static,
    {
    ...
    where
        C: Fn(T::Arguments) -> T::Output + Send + 'static,
    {
        let previous = self
            .closure
    ...
    #[doc(hidden)]
    pub fn __detour(&self) -> &dyn Fn(T::Arguments) -> T::Output {
